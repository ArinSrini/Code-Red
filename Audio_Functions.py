...
def stft(audio_signal, window_function, step_length):
    ...
    Example: Compute and display the spectrogram from an audio file.
        ...
        audio_signal, sampling_frequency = Audio_Functions.wavread("audio_file.wav")
        ...
        # Compute the STFT
        audio_stft = Audio_Functions.stft(audio_signal, window_function, step_length)
        ...
        Audio_Functions.specshow(audio_spectrogram, number_samples, sampling_frequency, xtick_step=1, ytick_step=1000)
...
def istft(audio_stft, window_function, step_length):
    ...
    Example: Estimate the center and the sides from a stereo audio file.
        ...
        audio_stft1 = Audio_Functions.stft(audio_signal[:, 0], window_function, step_length)
        audio_stft2 = Audio_Functions.stft(audio_signal[:, 1], window_function, step_length)
        ...
        center_signal1 = Audio_Functions.istft(center_stft1, window_function, step_length)
        center_signal2 = Audio_Functions.istft(center_stft2, window_function, step_length)
        ...
        Audio_Functions.wavwrite(center_signal, sampling_frequency, "center_file.wav")
        Audio_Functions.wavwrite(sides_signal, sampling_frequency, "sides_file.wav")

        # Display the original, center, and sides signals in seconds
        xtick_step = 1
        plt.figure(figsize=(14, 7))
        plt.subplot(3, 1, 1), Audio_Functions.sigplot(audio_signal, sampling_frequency, xtick_step)
        plt.ylim(-1, 1), plt.title("Original signal")
        plt.subplot(3, 1, 2), Audio_Functions.sigplot(center_signal, sampling_frequency, xtick_step)
        plt.ylim(-1, 1), plt.title("Center signal")
        plt.subplot(3, 1, 3), Audio_Functions.sigplot(sides_signal, sampling_frequency, xtick_step)
...
def melfilterbank(sampling_frequency, window_length, number_filters):
    ...
    Example: Compute and display the mel filterbank.
        ...
        window_length = pow(2, int(np.ceil(np.log2(0.04 * sampling_frequency))))
        number_mels = 128
        mel_filterbank = Audio_Functions.melfilterbank(sampling_frequency, window_length, number_mels)
...
def melspectrogram(audio_signal, window_function, step_length, mel_filterbank):
    ...
    Example: Compute and display the mel spectrogram.
        ...
        mel_spectrogram = Audio_Functions.melspectrogram(audio_signal, window_function, step_length, mel_filterbank)

        # Display the mel spectrogram in dB, seconds, and Hz
        number_samples = len(audio_signal)
        plt.figure(figsize=(14, 5))
        Audio_Functions.melspecshow(mel_spectrogram, number_samples, sampling_frequency, window_length, xtick_step=1)
...
def mfcc(
    ...
    Example: Compute and display the MFCCs, delta MFCCs, and delta-delta MFCCs.
        ...
        audio_mfcc = Audio_Functions.mfcc(audio_signal, window_function, step_length, mel_filterbank, number_coefficients)
        ...
        plt.figure(figsize=(14, 7))
        plt.subplot(3, 1, 1)
        Audio_Functions.mfccshow(audio_mfcc, number_samples, sampling_frequency, xtick_step), plt.title("MFCCs")
        plt.subplot(3, 1, 2)
        Audio_Functions.mfccshow(audio_dmfcc, number_samples, sampling_frequency, xtick_step), plt.title("Delta MFCCs")
        plt.subplot(3, 1, 3)
        Audio_Functions.mfccshow(audio_ddmfcc, number_samples, sampling_frequency, xtick_step), plt.title("Delta-delta MFCCs")
        plt.tight_layout()
        plt.show()
    """
...
def cqtkernel(
    ...
    Example: Compute and display a CQT kernel.
        ...
        cqt_kernel = Audio_Functions.cqtkernel(sampling_frequency, octave_resolution, minimum_frequency, maximum_frequency)
...
def cqtspectrogram(audio_signal, sampling_frequency, time_resolution, cqt_kernel):
    ...
    Example: Compute and display the CQT spectrogram.
        ...
        time_resolution = 25
        cqt_spectrogram = Audio_Functions.cqtspectrogram(audio_signal, sampling_frequency, time_resolution, cqt_kernel)

        # Display the CQT spectrogram in dB, seconds, and Hz
        plt.figure(figsize=(14, 5))
        Audio_Functions.cqtspecshow(cqt_spectrogram, time_resolution, octave_resolution, minimum_frequency, xtick_step=1)
...
def cqtchromagram(
    ...
    Example: Compute and display the CQT chromagram.
        ...
        time_resolution = 25
        cqt_chromagram = Audio_Functions.cqtchromagram(audio_signal, sampling_frequency, time_resolution, octave_resolution, cqt_kernel)

        # Display the CQT chromagram in seconds
        plt.figure(figsize=(14, 3))
        Audio_Functions.cqtchromshow(cqt_chromagram, time_resolution, xtick_step=1)
...
def dct(audio_signal, dct_type):
    ...
    Example: Compute the 4 different DCTs and compare them to SciPy's DCTs.
        ...
        audio_dct1 = Audio_Functions.dct(audio_segment, 1)
        audio_dct2 = Audio_Functions.dct(audio_segment, 2)
        audio_dct3 = Audio_Functions.dct(audio_segment, 3)
        audio_dct4 = Audio_Functions.dct(audio_segment, 4)
...
def dst(audio_signal, dst_type):
    ...
    Example: Compute the 4 different DSTs and compare their respective inverses with the original audio.
        ...
        audio_dst1 = Audio_Functions.dst(audio_segment, 1)
        audio_dst2 = Audio_Functions.dst(audio_segment, 2)
        audio_dst3 = Audio_Functions.dst(audio_segment, 3)
        audio_dst4 = Audio_Functions.dst(audio_segment, 4)

        # Compute their respective inverses, i.e., DST-I, II, III, and IV
        audio_idst1 = Audio_Functions.dst(audio_dst1, 1)
        audio_idst2 = Audio_Functions.dst(audio_dst2, 3)
        audio_idst3 = Audio_Functions.dst(audio_dst3, 2)
        audio_idst4 = Audio_Functions.dst(audio_dst4, 4)
...
def mdct(audio_signal, window_function):
    ...
    Example: Compute and display the MDCT as used in the AC-3 audio coding format.
        ...
        # Compute the MDCT
        audio_mdct = Audio_Functions.mdct(audio_signal, window_function)

        # Display the MDCT in dB, seconds, and Hz
        number_samples = len(audio_signal)
        plt.figure(figsize=(14, 7))
        Audio_Functions.specshow(np.absolute(audio_mdct), number_samples, sampling_frequency, xtick_step=1, ytick_step=1000)
        plt.title("MDCT (dB)")
        plt.tight_layout()
        plt.show()
    """
...
def imdct(audio_mdct, window_function):
    ...
    Example: Verify that the MDCT is perfectly invertible.
        ...
        audio_signal2 = Audio_Functions.imdct(audio_mdct, window_function)
        ...
        plt.subplot(3, 1, 1), Audio_Functions.sigplot(audio_signal, sampling_frequency, xtick_step)
        plt.ylim(-1, 1), plt.title("Original signal")
        plt.subplot(3, 1, 2), Audio_Functions.sigplot(audio_signal2, sampling_frequency, xtick_step)
        plt.ylim(-1, 1), plt.title("Resyntesized signal")
        plt.subplot(3, 1, 3), Audio_Functions.sigplot(audio_differences, sampling_frequency, xtick_step)
